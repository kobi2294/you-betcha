...
function inProgressMatchVm(
  ...
  const userGuesses = userScores.map((model) => userGuess(users, model));
...
function pastMatchVm(
  ...
  const userScores = (calculatedGroupMatchScore?.userScores ?? []).map(
    (model) => userScore(users, model, currectGuess, pointsEach, correctCount)
  );
...
export function buildTable(
  group: DbModel.CalculatedGroup | null,
  pastMatches: PastMatchVm[]
): GroupTableVm {
  if (!group) {
    return [];
  }
  ...
  const rows = group.users.map((user) =>
    buildRow(user, userScores[user.id] ?? [])
  );
  return rows;

  function buildRow(
    user: DbModel.GroupUserRecord,
